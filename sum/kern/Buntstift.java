...
public class Buntstift extends Stift
{
	...
	/**
	 Der Buntstift wird als Stift initialisiert und mit den
	...
	public void zeichneRechteck(double pBreite, double pHoehe)
	{
		...
		if (g != null)
		{
			...
			if (zMuster == Muster.DURCHSICHTIG)
				...
				g.fill(new Rectangle2D.Double(zStiftH, zStiftV, pBreite, pHoehe));
		}
	}
	...
	public void zeichneKreis(double pRadius)
	{
		...
		if (g != null)
		{
			...
			if (zMuster == Muster.DURCHSICHTIG)
				...
				g.fill(new Ellipse2D.Double(zStiftH - pRadius, zStiftV - pRadius, 2 * pRadius, 2 * pRadius));
		}
	}
	...
	protected void setzeZustand(Graphics2D g)
	{
		if (zMuster == Muster.DURCHSCHEINEND
			...
			zFarbe = new Color(zFarbe.getRed(), zFarbe.getGreen(), zFarbe.getBlue(), 128);
		else
			if (zMuster != Muster.DURCHSCHEINEND
					&& zFarbe.getTransparency() != Color.OPAQUE)
				zFarbe = new Color(zFarbe.getRed(), zFarbe.getGreen(), zFarbe.getBlue());
